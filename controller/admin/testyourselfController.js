...
const CreatTest = async (req, res) => {
    try {
      ...
      if (response) {
        ...
        res
          .status(400)
          .json({ success: false, message: "Test not Created" });
      }
    } catch (error) {
  ...
  const deleteTest = async (req, res) => {
    try {
      ...
      if (response) {
        res
          .status(200)
          .json({ success: true, message: "Test deleted" });
      } else {
  ...
  const CreateTestQuestions = async (req, res) => {
    ...
    try {
      ...
      if (!question) {
        return res.status(400).json({ success: false, message: "Test Question not Created" });
      }
